...
void DrawBullet(SDL_Renderer *renderer, Bullet bullet, SpatialShip spatial_ship) {
    int radius = CalculDrawRadius(bullet.position, bullet.radius, spatial_ship);
    Point bullet_screen_position = CalculObjectScreenPosition(bullet.position, spatial_ship);
...
void DrawAsteroid(SDL_Renderer *renderer, Asteroid asteroid, SpatialShip spatial_ship) {
    int radius = CalculDrawRadius(asteroid.position, asteroid.radius, spatial_ship);
    Point asteroid_screen_position = CalculObjectScreenPosition(asteroid.position, spatial_ship);
...
Point* GetVisibleBlocks(SpatialShip spatial_ship) {
    ...
    Point* blocks = malloc(4 * sizeof(Point));
    blocks[0] = (Point) {pos_x, pos_y};
    blocks[1] = (Point) {pos_x + 1, pos_y};
    blocks[2] = (Point) {pos_x, pos_y + 1};
    blocks[3] = (Point) {pos_x + 1, pos_y + 1};
    return blocks;
}
// fonction qui retourne les object de la scène à afficher
Asteroid* GetBlocks(Asteroid*** asteroids, SpatialShip spatial_ship) {
    Asteroid* scene_object = malloc(16 * sizeof(Asteroid));
    Point* blocks = GetVisibleBlocks(spatial_ship);
    for (int i = 0; i < 4; i++) {
        Point block = blocks[i];
        Asteroid* block_asteroids = asteroids[block.y][block.x];
        for (int j = 0; j < 4; j++) {
            Asteroid asteroid = block_asteroids[j];
            scene_object[(i*j)+j] = asteroid;
        }
    }
    free(blocks);
    return scene_object;
...
Asteroid* CalculAsteroidScreenPosition(Asteroid* scene_object, SpatialShip spatial_ship) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        scene_object_screen_position[i].position = CalculObjectScreenPosition(asteroid.position, spatial_ship);
...
Bullet* CalculBulletScreenPosition(Bullet* bullets, int bullet_size, SpatialShip spatial_ship) {
    Bullet* bullets_screen_position = malloc(bullet_size * sizeof(Bullet));
    for (int i = 0; i < bullet_size; i++) {
        Bullet bullet = bullets[i];
        bullets_screen_position[i].position = CalculObjectScreenPosition(bullet.position, spatial_ship);
    }
...
void Scene(SDL_Renderer *renderer, Asteroid*** asteroids, Bullet* bullets, int bullet_number, SpatialShip ship) {
    ...
    // -------------------------------------------------------------------------------------------------------------------------------------------------------
    // Asteroid* scene_object_asteroids = GetBlocks(asteroids, ship);
    int pos_y = (int) (ship.position.y / wnd_size.y);
    int pos_x = (int) (ship.position.x / wnd_size.x);
        if (bullets != NULL) {
            printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 11 vitesse: %d\n", bullets[0].speed);
        }
    Point* blocks = malloc(4 * sizeof(Point));
    ...
    Asteroid* scene_object_asteroids = malloc(16 * sizeof(Asteroid));
        if (bullets != NULL) {
            printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 14 vitesse: %d\n", bullets[0].speed);
        }
    for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            Asteroid asteroid = block_asteroids[j];
            scene_object_asteroids[(i*j)+j] = asteroid;
        }
    }
        if (bullets != NULL) {
            printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 15 vitesse: %d\n", bullets[0].speed);
    ...
    Asteroid* screen_position_asteroids = CalculAsteroidScreenPosition(scene_object_asteroids, ship);
        if (bullets != NULL) {
            printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 17 vitesse: %d\n", bullets[0].speed);
        }
    ...
    Bullet* screen_position_bullets = CalculBulletScreenPosition(bullets, bullet_number, ship);
        if (bullets != NULL) {
    ...
    for (int i = 0; i < 16; i++) {
        for (int j = bullets_index; j < bullet_number; j++) {
            if (sorted_bullets[j].position.y < sorted_asteroids[i].position.y) {
                DrawBullet(renderer, sorted_bullets[j], ship);
                bullets_index++;
            }
        }
        ...
        DrawAsteroid(renderer, sorted_asteroids[i], ship);
    }
    for (int i = bullets_index; i < bullet_number; i++) {
        DrawBullet(renderer, sorted_bullets[i], ship);
    }
    if (!ship_drawed) {
        DrawSpatialShip(renderer, ship);
    }
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_ESCAPE) {
                ...
                if (SDL_GetTicks() - last_bullet_shot > 500) {
                    ...
                    // Shoot a bullet
                    bullet_number++;
                    bullets = addBullet(bullets, bullet_number, spatial_ship);
                }
            }
        }
        // PHYSIC
